...
@property (nonatomic,strong)GroupBuyListView *mainView;
...
- (void)viewDidLoad
{
    self.viewModel = [GroupBuyListViewModel new];
    [super viewDidLoad];
    [self addViews];
    [self bindBaseSignal];
...
{
    // 导航
    self.navigationBarView.titleLabel.text = @"懒店拼团";
    // 主视图
    self.mainView = [GroupBuyListView new];
...
- (void)bindBaseSignal
{
    ...
    [self.viewModel.updatedContentSignal subscribeNext:^(id x) {
        ...
        switch (self.viewModel.currentSignalType) {
            case GroupBuyListViewModel_Signal_Type_GotoGoodsDetail:
            {
                // 商品详情
