...
#import "BargainProgressViewController.h"
#import "ConfirmOrderViewController.h"

@interface MyBargainViewController ()

@property (nonatomic,strong)MyBargainViewModel *viewModel;
@property (nonatomic,strong)MyBargainView *mainView;

@end

@implementation MyBargainViewController

- (void)viewDidLoad
{
    self.viewModel = [MyBargainViewModel new];
    [super viewDidLoad];
    [self addViews];
    [self bindSignal];
}
...
{
    // 导航
    self.navigationBarView.titleLabel.text = @"我的砍价";
    ...
    // 主视图
    self.mainView = [MyBargainView new];
...
- (void)bindSignal
{
    ...
    [self.viewModel.updatedContentSignal subscribeNext:^(id x) {
        ...
        switch (self.viewModel.currentSignalType) {
            case MyBargainViewModel_Signal_Type_GotoBargainDetail:
            {
                // 跳转砍价进度
                NSArray *params = x;
                BargainProgressViewController *vc = [BargainProgressViewController new];
                ...
                break;
            case MyBargainViewModel_Signal_Type_GotoConfirmOrder:
            {
                // 跳转确认下单
