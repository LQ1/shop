...
- (void)LQPhotoPicker_initPickerView{
    ...
    self.pickerCollectionView = [[UICollectionView alloc]initWithFrame:rc collectionViewLayout:layout];
    
    if (_LQPhotoPicker_superView) {
        NSArray *arraySubViews = [_LQPhotoPicker_superView subviews];
        for (UIView *viewSub in arraySubViews) {
            if ([viewSub isKindOfClass:[self.pickerCollectionView class]]) {
    ...
    //添加图片提示
    addImgStrLabel = [[UILabel alloc]initWithFrame:CGRectMake(100, 50, 180, 20)];
    addImgStrLabel.text = [NSString stringWithFormat:@"添加图片最多%ld张",_LQPhotoPicker_imgMaxCount];
