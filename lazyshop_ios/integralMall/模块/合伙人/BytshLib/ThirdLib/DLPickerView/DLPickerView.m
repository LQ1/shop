...
@property (nonatomic,copy)CheckDataBlock checkDataBlock;
...
- (instancetype)initWithPlistName:(NSString *)plistName
                ...
                withSelectedBlock:(SelectedBlock)selectedBlock
               withCheckDataBlock:(CheckDataBlock)checkDataBlock{
    self.checkDataBlock = checkDataBlock;
    return [self initWithPlistName:plistName withSelectedItem:selectedData withSelectedBlock:selectedBlock];
}
...
- (void)confirm {
    if (self.checkDataBlock) {
        if (self.isSingleColumn) {
            if(!self.checkDataBlock([self.selectedItem copy]))return;
        } else {
            if(!self.checkDataBlock([self.selectedItems copy]))return;
        }
    }
    [self hide:^{
