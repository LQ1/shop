...
//FB0514
#define kDefaultIndicatorColor [UIColor colorWithRed:0xFB/255.0f green:0x05/255.0f blue:0x14/255.0f alpha:0.0f]
//背影颜色
#define kDefaultTabsViewBackgroundColor [UIColor clearColor]
#define kDefaultContentViewBackgroundColor [UIColor colorWithRed:104/255.0f green:152/255.0f blue:134/255.0f alpha:0.0f]
...
- (void)drawRect:(CGRect)rect {
    ...
    //[[UIColor colorWithWhite:0/255.0 alpha:0.75] setStroke];
    ...
    if (self.selected) {
        ...
        //线条颜色
        [[UIColor colorWithRed:0xED/255.0f green:0x6D/255.0f blue:0x00/255.0f alpha:1.0f] setStroke];
        [bezierPath stroke];
    }
}
@end
...
- (void)viewWillLayoutSubviews {
    ...
    frame.origin.x = 0.0;
    frame.origin.y = 20 + 42;//self.tabLocation ? 0.0 : self.view.frame.size.height - self.tabHeight;
    ...
    frame.origin.x = 0.0;
    frame.origin.y = 0;//self.tabLocation ? self.tabHeight : 0.0;
    frame.size.width = self.view.bounds.size.width;
    if (self.HasBottom) {
...
- (void)reloadData {
    ...
    if (_tabCount != 0 && _tabCount<=5) {//最多一页显示5个标签页，否则直接用默念标签页的宽度
        _tabWidth = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabWidth withDefault:[UIScreen mainScreen].bounds.size.width/_tabCount];
    ...
    if (self.isStartWithUserIndex) {
        viewController = [self viewControllerAtIndex:self.userIndex];
    }
