...
{
    if (!outSideBtn) {
        outSideBtn = [[UIButton alloc] initWithFrame:SuperView.bounds];
        outSideBtn.backgroundColor = [UIColor blackColor];
        outSideBtn.alpha = .5;
        [SuperView addSubview:outSideBtn];
        
        @weakify(self);
        outSideBtn.rac_command = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            @strongify(self);
            [self pause];
            ...
            [outSideBtn removeFromSuperview];
...
- (void)layoutSubviews{
    ...
    loadActivity.center= CGPointMake(self.bounds.size.width*.5, self.bounds.size.height*.5);
}
