...
{
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(shouldSelectAsset:isSelected:)]) {
        return [_delegate shouldSelectAsset:_asset isSelected:select];
    }
    return YES;
}
...
-(BOOL)shouldSelectAsset:(ALAsset *)asset isSelected:(BOOL)select
{
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(shouldSelectAsset:isSelected:)]) {
        return [_delegate shouldSelectAsset:asset isSelected:select];
    }
    return YES;
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (!unFirst) {
        self.columns=floor(self.view.frame.size.width/(kThumbnailSize.width+minimumInteritemSpacing));
...
- (void)setupAssets
{
    ...
    __block typeof(self) selfCp = self;
    
    ALAssetsGroupEnumerationResultsBlock resultsBlock = ^(ALAsset *asset, NSUInteger index, BOOL *stop) {
        
        if (asset)
        {
            [selfCp.assets addObject:asset];
            
            NSString *type = [asset valueForProperty:ALAssetPropertyType];
            
            if ([type isEqual:ALAssetTypePhoto])
                selfCp.numberOfPhotos ++;
            if ([type isEqual:ALAssetTypeVideo])
                selfCp.numberOfVideos ++;
        }
        
        else if (selfCp.assets.count > 0)
        {
            [selfCp.tableView reloadData];

            [selfCp.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:ceil(selfCp.assets.count*1.0/selfCp.columns)  inSection:0] atScrollPosition:UITableViewScrollPositionTop animated:YES];
        }
    };
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==ceil(self.assets.count*1.0/self.columns)) {
    ...
    for (int i=0; i<self.columns; i++) {
        if ((indexPath.row*self.columns+i)<self.assets.count) {
            [tempAssets addObject:[self.assets objectAtIndex:indexPath.row*self.columns+i]];
        }
    }
    ...
    [cell bind:tempAssets selectionFilter:picker.selectionFilter minimumInteritemSpacing:minimumInteritemSpacing minimumLineSpacing:minimumLineSpacing columns:self.columns assetViewX:(self.tableView.frame.size.width-kThumbnailSize.width*tempAssets.count-minimumInteritemSpacing*(tempAssets.count-1))/2];
    return cell;
}

-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return ceil(self.assets.count*1.0/self.columns)+1;
}
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset isSelected:(BOOL)select
{
    ...
    if (!select && _indexPathsForSelectedItems.count+1 > vc.maximumNumberOfSelection) {
